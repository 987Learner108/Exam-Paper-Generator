...
import { Upload, FileText, PlusCircle, History, BarChart3, Trash2 } from 'lucide-react'
...
const TeacherDashboard = () => {
  ...
  const handleDeleteResource = async (resourceId, filename) => {
    if (!confirm(`Are you sure you want to delete "${filename}"? This will also remove its content from the knowledge base.`)) {
      return
    }
  ...
  const bloomsData = papers.reduce((acc, paper) => {
    Object.entries(paper.blooms_distribution || {}).forEach(([level, count]) => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Generate and manage exam papers with AI</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Link to="/upload" className="card hover:shadow-lg transition-shadow cursor-pointer">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Upload Resources</h3>
                <p className="text-sm text-gray-600">Add syllabus & materials</p>
              </div>
            </div>
          </Link>

          <Link to="/generate" className="card hover:shadow-lg transition-shadow cursor-pointer">
            <div className="flex items-center space-x-4">
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{paper.department}</p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm text-gray-600 mt-1">{paper.total_marks} marks</p>
                      </div>
                    </div>
                  </Link>
                ))}
            ...
            {resources.length === 0 ? (
              ...
              <div className="space-y-2">
                {resources.slice(0, 5).map((resource) => (
                  <div key={resource.id} className="p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div className="flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {resource.subject || 'No subject'} • {(resource.file_size / 1024).toFixed(0)} KB
                          {resource.uploaded_by && ` • by ${resource.uploaded_by}`}
                        </p>
                      </div>
